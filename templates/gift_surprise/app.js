(function () {
  ...
  function populate() {
    document.title = `Surprise — ${data.recipientName || 'You'}`
    setText('gift-overline', data.overline || 'Mystery Awaits')
    setText('gift-recipient', data.recipientName || 'คุณ')
    setText('msg-occasion', data.occasion || 'Surprise!')
    ...
    setText('finale-text', `ขอบคุณที่เปิดรับเซอร์ไพรส์นี้ — ${data.senderName || ''}`)
  ...
  function buildGallery() {
    ...
    grid.innerHTML = ''
    if (!photos.length) {
      grid.innerHTML = '<div class="gallery-item active"><div class="gallery-placeholder">✦ Surprise ✦</div></div>'
  ...
  function openGift() {
    ...
    setTimeout(() => showScreen('message'), 1400)
  }

  document.getElementById('envelope-scene').addEventListener('click', openGift)
  document.getElementById('envelope-scene').addEventListener('keydown', e => { if (e.key==='Enter'||e.key===' ') openGift() })

...
export function MemoryPageAdminFields({
  ...
  const updateEntry = (index: number, patch: Partial<MemoryPageEntry>) => {
    set({ entries: entries.map((e, i) => (i === index ? { ...e, ...patch } : e)) })
  }
  ...
  return (
    <div className="mp-admin">
      <AdminSection
        step={1}
        title="หัวหน้า Memory Page"
        scene="Hero header — ชื่อ + ประโยคนำ"
        help="ชื่อผู้รับ/ผู้ส่งมาจากช่องด้านบนฟอร์ม"
      >
      ...
      <AdminSection
        ...
        scene="Scrapbook scroll — รูป + สถานที่ + โน้ตลับ"
        help="แต่ละรูป = 1 ช่วงเวลา — ใต้รูปใส่สถานที่ โน้ตลับใส่ความรู้สึก"
      >
        <div className="mp-admin-entries">
          {entries.map((entry, i) => (
            <div key={entry.id || i} className="mp-admin-entry">
              ...
              <div className="form-group">
                <label>ความรู้สึก (เปิดซองอ่าน)</label>
                <textarea
        ...
        {entries.length < 20 ? (
          <button type="button" className="btn-luxury" onClick={() => set({ entries: [...entries, blankEntry()] })}>
            + เพิ่มช่วงความทรงจำ
          </button>
        ) : null}
      </AdminSection>
      ...
      <AdminSection
        step={3}
        title="จดหมายขอบคุณท้ายหน้า"
        scene="Thank-you letter — เปิดซองอ่าน"
        help="ข้อความยาว ๆ หลังเลื่อนดูรูปครบ"
      >
        <div className="form-group">

...
export function LoveStoryAdminFields({
  ...
  const updateMemory = (index: number, patch: Partial<LoveStoryMemory>) => {
    set({ memories: memories.map((m, i) => (i === index ? { ...m, ...patch } : m)) })
  }

  const updateCapsule = (index: number, patch: Partial<LoveCapsule>) => {
    set({ capsules: capsules.map((c, i) => (i === index ? { ...c, ...patch } : c)) })
  }
  ...
  return (
    <div className="mp-admin">
      <AdminSection
        step={1}
        title="หน้าล็อกรหัส"
        scene="Password lock — ก่อนเข้าแดชบอร์ด"
        help="รูปคู่และเพลงแสดงบนหน้ากรอกรหัส"
      >
      ...
      <AdminSection
        step={2}
        title="แดชบอร์ด"
        scene="Love Dashboard — หน้าแรกหลังปลดล็อก"
        help="ชื่อเรื่องและเป้านับวันที่แสดงในหน้าแดชบอร์ด"
      >
        <div className="grid-2">
          <div className="form-group">
      ...
      <AdminSection
        step={3}
        title="Polaroid ความทรงจำ"
        scene="Memories gallery — การ์ดรูปในแดชบอร์ด"
        help="แต่ละการ์ด = 1 รูป + หัวข้อ + ข้อความ"
      >
        {memories.map((m, i) => (
          <div key={i} className="mp-admin-entry">
            <div className="mp-admin-entry-head">
              <strong>ความทรงจำที่ {i + 1}</strong>
              {memories.length > 1 ? (
                <button
                  type="button"
      ...
      <AdminSection
        step={4}
        title="ซองข้อความลับ (รายเดือน)"
        scene="Time capsules — เปิดตามวันครบรอบ"
        help="แต่ละซองมีข้อความ + รูปเมื่อเปิดอ่าน"
      >
        {capsules.map((c, i) => {
          ...
          return (
            <div key={c.id || i} className="mp-admin-entry">
              ...
              <AdminMediaField
                label="รูปในซอง (เมื่อเปิดอ่าน)"
                value={c.imageUrl || ''}
                onChange={(url) => updateCapsule(i, { imageUrl: url })}
              />
              <div className="form-group">
